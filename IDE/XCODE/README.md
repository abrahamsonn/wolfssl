...
Additionally, if otions not listed by default are included in the
`user_settings.h` file, the wolfSSL testsuite may fail when being built in
Xcode. The corresponding source file that implement that functionality will
need to be added to the list of compiled sources. This can be done with the
following steps:
...
3. Expand the "Compile Sources" option. You can see the list of the example
   ...
   location.
4. Select "+" and navigate to the location of the missing source file.
